...
class Dataentry extends MY_Controller {
    ...
    {       
        ...
        $page_data += [
            ...
            "content"           =>  [$this->load->view('interface/usermanagement/Dataentry', [
    ...
    function saveDataPrimary(){
        ...
        if(!$statusCOVID||!$dateEntried||!$barangayName||!$streetName||!$firstName||!$lastName||!$gender||!$personAge||!$personNumber||!$symptomsCOVID){
           $ret = ["success"   => false];
        }else{
    ...
    function saveDataContact(){
        ...
        // if(!$statusCOVID||!$dateEntried||!$barangayName||!$streetName||!$firstName||!$lastName||!$gender||!$personAge||!$relation){
        //    $ret = ["success"   => false];
        // }else{
            // foreach ($this->input->post("dataBarangay") as $key => $value2){
            // $data[] = [
    ...
    function getPersonCovid(){
        ...
        // foreach ($this->db->query("SELECT t1.barangay_id FROM tbl_user_barangay_district t1 WHERE t1.party_id=$district_id")->result() as $key => $value1) {
            ...
            foreach ($this->db->query("SELECT t1.*,t2.status_id,t3.travel_history,t3.date_arrival,t3.testing_code,t3.remarks,t3.whereabouts,t3.category,t3.office_id,t3.agency,t4.test_id,t4.result_id,t5.qstatus_id FROM tbl_person t1
                ...
                //                                 :"<a class='btn btn-success btn-xs ml-2 text-white' onclick='addContact($id,\"$fullName\")' style='cursor:pointer;'><i class='fa fa-plus'></i></a>";
                $tree = $this->hasContact($id)>0?"<center><span class='badge bg-danger text-md' onclick='myContact($id,\"$fullName\");removeAllEntry2();' style='cursor:pointer'>".$this->hasContact($id)."</span></center>":"";
                ...
                $thisTestCode2 = (!$value->testing_code?"":"<span class='badge bg-gray text-sm'>".$value->testing_code."</span>");
                $test = "<center><a class='btn btn-xs bg-yellow' style='white-space:nowrap;'><center style='font-size:12px;'>".(!$value->test_id?"NO DATA":$this->getStatusCode($value->test_id)."<br/>".$result)."</center></a></center>";
                $quarantine = "<center><a class='btn btn-xs bg-info bg-xs' style='white-space:nowrap;'><center>".(!$value->qstatus_id?"NO DATA":"<span class='badge bg-info'>".$this->getStatusName($value->qstatus_id)."</span>")."</center></a></center>";
                ...
                $data["data"][] = [
                    ...
                    "<center><a class='btn btn-xs text-white' style='white-space:nowrap;font-size:12px;background-color:".(!$value->status_id?"black":$this->getStatusColor($value->status_id)).";'><b>".(!$value->status_id?"NO DATA":$this->getStatusName($value->status_id))."</b></a></center>",
                    $test,
                    $quarantine,
    ...
    function getPersonCovidStatus(){
        ...
        foreach ($this->db->query("SELECT t1.*, to_char(t1.date_status, 'MON dd, YYYY') as date_status2 FROM tbl_covid_status_history t1 $query")->result() as $key => $value) {
            $data["data"][] = [
                $cc++,
                "<span class='btn btn-xs text-white' style='cursor:pointer;white-space:nowrap;font-size:12px;background-color:".(!$value->status_id?"black":$this->getStatusColor($value->status_id)).";'><b>".(!$value->status_id?"NO DATA":$this->getStatusName($value->status_id))."</b></span>",
